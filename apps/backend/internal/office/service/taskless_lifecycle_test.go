...
func TestTasklessUsageDuplicate(t *testing.T) {
	...
	payload := lifecycle.AgentStreamEventPayload{
		...
		Data: &lifecycle.AgentStreamEventData{PromptGeneration: 1, TurnID: "turn-1", CurrentModelID: "test-model", Usage: &streams.PromptUsage{InputTokens: 7, OutputTokens: 3, OutputTokensPresent: true, TotalTokens: 10}},
	}
	for i := 0; i < 2; i++ {
		if err := eb.Publish(ctx, events.BuildAgentStreamSubject("run-session-1"), bus.NewEvent(events.AgentStream, "test", payload)); err != nil {
